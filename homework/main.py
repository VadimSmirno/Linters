...
# TODO проще, конечно, сделать два отдельных эндпонита, ведь они очень разные по сути
async def get_recept(
        idx: Optional[int] = None
) -> List[dict]:
    async with session as db:
        if not idx:
            recept = await db.execute(select(Recipes). \
...
async def create_recept(recept: schemas.RecipesSchema):
    ...
    ingredient = [Ingredient(name=ingredient.get('name'), recipe=new_recipe) for ingredient in recept.ingredients_list]
